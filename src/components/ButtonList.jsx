...
const ButtonList = () => {
  ...
  return (
    <div className="flex items-center relative mr-5 ">
      ...
      <div className={` overflow-x-scroll overflow-y-hidden whitespace-nowrap  ${isMenuOpen ? " max-2xl:w-[80rem] max-xl:w-[52rem] max-lg:w-[34rem] max-md:w-[22rem] ]" : "xl:w-[98rem]" } max-sm:w-[22rem]  `}>
        {buttonList.map((name, index) => (
          <ul key={index} className="inline-block">
            <li className={`text-center  bg-gray-200 py-2 px-5 rounded-md mr-3 font-semibold cursor-pointer  ${activeButton === name ? "bg-gray-600 text-white" : " hover:bg-gray-300 transition ease-in-out delay-100"}`} onClick={() => dispatch(activeButtonFilterHandler(name))}>
              {name}
            </li>
          </ul>
        ))}
